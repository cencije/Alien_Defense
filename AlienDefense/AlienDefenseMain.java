...
public class AlienDefenseMain extends GraphicsProgram implements Runnable 
{
  ...
  private Color pathColor = new Color(224,236,255, 180);
  ...
  private int numberOfLives, totalLives, dollorpsEarned, totalMoney;
  ...
  // Game Play settings
  private GImage livesIcon, moneyIcon;
  private GLabel lifeTotal, moneyTotal;
  ...
  public void mouseEntered(MouseEvent e) {
    ...
    Image cImg = tk.getImage("Images/TargetSight.png");
    Point p = new Point(0,0);
    Cursor c = tk.createCustomCursor(cImg, p, "Cursor");
    this.setCursor(c);
  }
  ...
  public void drawMap(int mapNumber) {
    ...
    moneyIcon = new GImage("Images/Dollar_Sign.png");
    moneyIcon.setSize(10,10);
    add(moneyIcon, 505, 10);
    totalMoney = 300;
    moneyTotal = new GLabel(""+ totalMoney);
    moneyTotal.setFont(new Font("Serif", Font.PLAIN, 14));
    moneyTotal.setColor(Color.WHITE);
    add(moneyTotal, 518, 20);
    if (mapNumber == 1) {
  ...
  public void drawTowers() {
    ...
    add(tower1, 501, 50);
    if (playerLevel >= 3) {
    tower3 = new GImage("Images/Towe3.png");
    add(tower3,501,150);
    }
    if (playerLevel >= 6) {
  ...
  public void towerShoot(Tower shootingTower, int towerType) {
	  if (towerType == 1) {
		  Bullet ammoBullet = new Bullet(10, 2, -10, 0, this);
		  add(ammoBullet, shootingTower.getX(), shootingTower.getY()+50);
		  new Thread(ammoBullet).start();
	  }
